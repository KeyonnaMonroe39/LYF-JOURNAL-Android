package com.keyonnamonroe.lyfjournal.framework.android.http.okhttp.internal
...
internal class RequestCleanupStrategy : RequestCleanupSpec() {
    ...
    override fun onStateChanged(state: RequestState) {
        ...
        when (currentRequestState) {
            RequestState.Cancelled, RequestState.Failed, RequestState.Successful -> releaseResources()
            else -> {
                // no op
            }
        }
